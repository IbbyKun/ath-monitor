...
| 5.1a | **Leave "Half Day" blank unless you mean it** | 🟡 | — | `TimeCalculator` derives the full-day requirement as `halfDay × 2`, *not* from the shift length. Set Half Day to 04:00 on a 09:00–18:00 shift and a full day silently becomes 8 h against a 9 h shift, so people are marked `H` for working their contracted hours. Blank makes it follow the shift (`shiftWorkTime / 2 × 2`). Worth a UI hint, or deriving the full-day figure from the shift independently. |
| 5.1b | Two independent 8-hour targets | 🟢 | 1 h | The shift's **Full-Day Productive Time** and the org-level **Monitoring Control → productive hours** both express "how much productive time a day needs", are set in different screens, and do not read each other. Nothing is broken, but they can disagree and the dashboard and attendance sheet will then tell different stories. Decide which is authoritative and document it, or fold one into the other. |
