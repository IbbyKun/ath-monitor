...
| ~~5.1a~~ | ~~Leave "Half Day" blank unless you mean it~~ | ✅ | done | Resolved by simplifying the rule rather than documenting the trap: half-day is now derived from the single target, and both half-day inputs are gone from the dialog. |
| ~~5.1b~~ | ~~Two productive-time targets~~ | ✅ | moot | Attendance no longer looks at productive time at all, so there is nothing to reconcile. The shift's hours decide attendance; Monitoring Control's productive-hours target drives the dashboard gauge. Different questions, different numbers. |
| 5.4 | `productivity_present` / `productivity_halfday` are now dead columns | 🟢 | 30 min | Nothing reads them since attendance moved to timer time. Still written by the shift save, and still in the schema. Drop from the payload and the table when convenient. |
| 5.5 | Per-day break lookup is one Mongo query per employee per day | 🟢 | 2 h | `fetchBreakSeconds` runs inside the day loop, so a 30-day sheet for 50 people is 1,500 aggregations. Fine at pilot scale, worth batching into one range query per employee before 300+. |
